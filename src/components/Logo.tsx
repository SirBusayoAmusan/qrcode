...
export const Logo: React.FC<LogoProps> = ({
  ...
  const content = (
    <div className={`inline-flex items-center gap-2.5 select-none transition-transform hover:opacity-95 ${className}`}>
      {/* Brand Icon */}
      <div className={`${iconSizes[size]} relative rounded-lg bg-gradient-to-br from-violet-600 to-indigo-600 p-0.5 shadow-md shadow-violet-500/25 flex items-center justify-center shrink-0`}>
      ...
      {showText && (
        <span className={`${textSizes[size]} text-white tracking-tight flex items-center`}>
          Clearpath<span className="text-violet-400">QR</span>
        </span>
      )}
    </div>
  );
