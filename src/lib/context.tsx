...
import { Channel, TapframePage, Lead, UserProfile } from '../types';
import { INITIAL_CHANNELS, INITIAL_PAGES, INITIAL_LEADS } from './demoData';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [channels, setChannels] = useState<Channel[]>(() => {
    const saved = localStorage.getItem('clearpath_channels');
    return saved ? JSON.parse(saved) : INITIAL_CHANNELS;
  });
  const [activeChannelId, setActiveChannelId] = useState<string>(() => {
    return localStorage.getItem('clearpath_active_channel_id') || INITIAL_CHANNELS[0].id;
  });
  const [pages, setPages] = useState<TapframePage[]>(() => {
    const saved = localStorage.getItem('clearpath_pages');
    return saved ? JSON.parse(saved) : INITIAL_PAGES;
  });
  const [leads, setLeads] = useState<Lead[]>(() => {
    const saved = localStorage.getItem('clearpath_leads');
  ...
  useEffect(() => {
    localStorage.setItem('clearpath_channels', JSON.stringify(channels));
  }, [channels]);

  useEffect(() => {
    localStorage.setItem('clearpath_pages', JSON.stringify(pages));
  }, [pages]);

  useEffect(() => {
    localStorage.setItem('clearpath_leads', JSON.stringify(leads));
  }, [leads]);

  useEffect(() => {
    if (activeChannelId) {
      localStorage.setItem('clearpath_active_channel_id', activeChannelId);
    }
  }, [activeChannelId]);
  ...
  useEffect(() => {
    ...
    async function initAuth() {
      try {
        ...
        console.warn('Supabase auth getSession notice (using fallback local state):', err);
      } finally {
  ...
  // Try to load user data from Supabase workflows/channels table if present
  const syncWithSupabase = async (userId: string) => {
    try {
      ...
      if (!error && data && data.data) {
        const payload = data.data as { channels?: Channel[]; pages?: TapframePage[]; leads?: Lead[] };
        if (payload.channels && payload.channels.length > 0) setChannels(payload.channels);
        if (payload.pages && payload.pages.length > 0) setPages(payload.pages);
        if (payload.leads && payload.leads.length > 0) setLeads(payload.leads);
      }
    } catch (err) {
      console.warn('Supabase sync skipped, using persistent local store', err);
    }
  };

  const persistToRemote = async (newChannels: Channel[], newPages: TapframePage[], newLeads: Lead[]) => {
    if (!user) return;
    try {
      await supabase
        .from('workflows')
        .upsert(
          {
            ...
            current_step: 4,
            data: { channels: newChannels, pages: newPages, leads: newLeads },
  ...
  const createPage = async (pageData: Partial<TapframePage>): Promise<TapframePage> => {
    ...
    const newPage: TapframePage = {
      ...
      slug: pageData.slug || ('page-' + Math.random().toString(36).substring(2, 7)),
      ...
      hero_image_url: pageData.hero_image_url || '/assets/youtube-creator-male.png',
      ...
      cta_buttons: pageData.cta_buttons || [
        { id: 'btn-d1', label: 'Explore Main Website', url: 'https://clearpathqr.com', variant: 'primary' }
      ],
      ...
      custom_theme: pageData.custom_theme || {
        ...
        qr_style: {
          ...
          callout_text: 'Scan the QR code to get instant access to the bonus materials',
        }
      },
  ...
  const addLead = async (leadData: Omit<Lead, 'id' | 'created_at'>) => {
    ...
    // Also increment lead count on the page
  ...
  const profile: UserProfile = {
    ...
    email: user?.email || 'creator@clearpathqr.com',
    full_name: user?.user_metadata?.full_name || 'Ali Creator',
    avatar_url: activeChannel?.avatar_url || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80',
    plan: 'free',
    channels,
  };
