...
export const PageEditor: React.FC = () => {
  ...
  const [headline, setHeadline] = useState(existingPage?.headline || '');
  ...
  const handleAddProductLink = () => {
    if (!isPro && productLinks.length >= 1) {
      setErrorMessage('Free tier is limited to 1 product link. Upgrade to Pro for unlimited links.');
      return;
    }
  ...
  const previewPage: TapframePage = {
    ...
    headline: headline || title || 'Get My Free Resource Kit',
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message === 'FREE_TIER_LIMIT_REACHED') {
        setErrorMessage('Free Tier limit reached. Upgrade to Pro for unlimited Tapframes.');
      } else {
        navigate('/dashboard');
      }
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          <form onSubmit={handleSave} className="space-y-6">
            ...
            <div className="p-6 rounded-2xl bg-[#11131E] border border-white/5 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">Unique collision-resistant slug guaranteed for this QR code.</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-[#11131E] border border-white/5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <div className="text-[11px] text-slate-400 mt-1">Collect visitor details and share downloads/product links.</div>
                </button>
              ...
              <div className="p-6 rounded-2xl bg-[#11131E] border border-white/5 space-y-5">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 🔥 Exclusive Viewer Offer"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Step-by-step systems, Notion templates, and toolkit as seen in the video."
                    className="w-full px-3.5 py-2 rounded-xl bg-[#0B0D15] border border-white/10 text-white text-xs placeholder:text-slate-600 focus:outline-none focus:border-violet-500"
                  />
                </div>

                {/* Product Links Section (Free tier: max 1 link, Pro tier: unlimited) */}
                <div className="p-4 rounded-xl bg-[#0B0D15] border border-white/5 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400">
                        {isPro ? 'Add multiple product & resource links.' : 'Free tier allows 1 product link.'}
                      </div>
                    </div>
                  ...
                  {productLinks.length === 0 ? (
                    <div className="p-3 text-center text-xs text-slate-500 border border-dashed border-white/10 rounded-xl">
                      No product links added yet. Click "+ Add Link" to attach products or Notion resources.
                    </div>
                  ) : (
                    <div className="space-y-3 pt-1">
                      {productLinks.map((link, idx) => (
                        <div key={link.id || idx} className="p-3 rounded-xl bg-[#11131E] border border-white/10 space-y-2 relative">
                          <div className="flex items-center justify-between">
                            <span className="text-[11px] font-bold text-violet-300">Link #{idx + 1}</span>
                            <button
                              type="button"
                          ...
                          <div>
                            <input
                              ...
                              placeholder="Link to the product (e.g. https://notion.so/...)"
            ...
            {/* 4. On-Screen QR Code Video Design */}
            <div className="p-6 rounded-2xl bg-[#11131E] border border-white/5 space-y-4">
              <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-violet-400">
          ...
          <div className="p-4 rounded-3xl bg-[#11131E] border border-white/10 flex flex-col items-center justify-center relative">
            <div className="mb-2">
              <Mascot 
                ...
                message="Your mobile landing page is streamlined and ready to capture viewer opt-ins!" 
              />
            </div>
            ...
            {previewTab === 'mobile' ? (
              <div className="w-full max-w-[320px] rounded-[36px] bg-[#090A0F] border-4 border-slate-700 shadow-2xl overflow-hidden p-4 text-center relative">
                ...
                <p className="text-[11px] text-slate-400 mb-4 line-clamp-3">
                  {subheadline || 'Subheadline and description of your resources.'}
                </p>

                {/* Lead Capture form preview */}
                {leadCaptureEnabled && (
                  <div className="p-3 rounded-xl bg-white/5 border border-white/10 mb-4 space-y-2 text-left">
                    <div className="text-[10px] font-bold text-violet-300 flex items-center gap-1">
                      <Sparkles className="w-3 h-3 text-emerald-400" />
                      <span>{leadMagnetTitle || 'Free Download for Viewers'}</span>
                    </div>
                ...
                {productLinks.length > 0 && (
                  <div className="space-y-1.5">
                    {productLinks.map((link, i) => (
                      <div
                        key={i}
                        className="py-2 px-3 rounded-xl bg-white/10 border border-white/5 text-xs font-semibold text-slate-200 flex items-center justify-between"
                      >
